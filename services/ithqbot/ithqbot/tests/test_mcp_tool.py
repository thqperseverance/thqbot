...
from ithqbot.agent.tools.mcp import MCPToolWrapper, connect_mcp_servers
from ithqbot.agent.tools.registry import ToolRegistry
from ithqbot.config.schema import MCPServerConfig
...
def _make_wrapper(
    session: object, *, timeout: float = 0.1, properties: dict[str, object] | None = None
) -> MCPToolWrapper:
...
@pytest.mark.asyncio
async def test_execute_injects_context_when_schema_declares_context_field() -> None:
    ...
    wrapper = _make_wrapper(
        ...
        properties={"value": {"type": "integer"}, "_ithqbot_ontext": {"type": "object"}},
    )
    ...
    assert isinstance(captured["_ithqbot_ontext"], dict)
    assert captured["_ithqbot_ontext"]["account_id"] == "acc_001"
    assert captured["_ithqbot_ontext"]["trace_id"] == "trace_001"
...
async def test_connect_mcp_servers_enabled_tools_warns_on_unknown_entries(
    ...
    monkeypatch.setattr("ithqbot.agent.tools.mcp.logger.warning", _warning)
