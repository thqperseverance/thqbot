...
def test_assistant_tool_calls_history_omits_null_content():
    session = Session(key="test:assistant-tool-calls-content")
    session.messages.append({
        "role": "assistant",
        "content": None,
        "tool_calls": [
    ...
    session.messages.append({"role": "tool", "tool_call_id": "call_a", "name": "x", "content": "ok"})

    history = session.get_history(max_messages=500)
    assert history[0]["role"] == "assistant"
    assert "content" not in history[0]
    assert history[0]["tool_calls"][0]["id"] == "call_a"
