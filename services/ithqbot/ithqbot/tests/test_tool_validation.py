...
class ContextSensitiveTool(Tool):
    @property
    def name(self) -> str:
    ...
    @property
    def description(self) -> str:
        return "requires optional context"
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        if context is None:
            return "ok-without-context"
        return str(context.get("trace_id", "ok-with-context"))


async def test_registry_does_not_inject_none_context() -> None:
    reg = ToolRegistry()
    reg.register(ContextSensitiveTool())
    result = await reg.execute("context_sensitive", {})
    assert result == "ok-without-context"


async def test_registry_accepts_none_params_with_validation_error() -> None:
    reg = ToolRegistry()
    reg.register(SampleTool())
    result = await reg.execute("sample", None)
    assert "参数不合法" in result


async def test_registry_tracks_direct_handler_with_tool_registration() -> None:
    async def _direct_handler(**_kwargs: Any) -> str:
        return "direct-ok"

    reg = ToolRegistry()
    reg.register(SampleTool(), direct_handler=_direct_handler)

    assert reg.get_direct_handler("sample") is _direct_handler
    assert reg.direct_route_names == ["sample"]
