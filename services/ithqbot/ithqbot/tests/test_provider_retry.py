...
from ithqbot.providers.base import GenerationSettings, LLMProvider, LLMResponse
...
@pytest.mark.asyncio
async def test_chat_with_retry_uses_structured_retryable_flag(monkeypatch) -> None:
    provider = ScriptedProvider([
        LLMResponse(content="opaque error", finish_reason="error", retryable=True, error_type="timeout"),
        LLMResponse(content="ok"),
    ])
    delays: list[int] = []
...
@pytest.mark.asyncio
async def test_chat_with_retry_uses_http_status_when_retryable_flag_missing(monkeypatch) -> None:
    provider = ScriptedProvider([
        LLMResponse(content="request failed", finish_reason="error", http_status=503),
        LLMResponse(content="ok"),
    ])
    delays: list[int] = []
