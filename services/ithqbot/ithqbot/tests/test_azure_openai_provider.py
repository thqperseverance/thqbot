...
async def test_chat_api_error():
    ...
    with patch("httpx.AsyncClient") as mock_client:
        ...
        assert isinstance(result, LLMResponse)
        assert "Azure OpenAI rejected the request (HTTP 401)." in result.content
...
async def test_chat_connection_error():
    ...
    with patch("httpx.AsyncClient") as mock_client:
        mock_context = AsyncMock()
        mock_context.post = AsyncMock(side_effect=ConnectError("Connection failed"))
        mock_client.return_value.__aenter__.return_value = mock_context
        ...
        assert isinstance(result, LLMResponse)
        assert "Azure OpenAI connection failed." in result.content
