"""测试 MemoryStore.consolidate() 处理非字符串工具参数的行为。

回归场景：当记忆压缩阶段收到 dict 类型的工具调用参数时，
应将其序列化为 JSON，而不是抛出 TypeError。
"""
...
from ithqbot.agent.memory import (
    MemoryStore,
    _MEMORY_STORE_FACTORIES,
...
class TestMemoryConsolidationTypeHandling:
    ...
    async def test_retries_transient_error_then_succeeds(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("ithqbot.providers.base.asyncio.sleep", _fake_sleep)
    ...
    async def test_tool_choice_fallback_on_unsupported_error(self, tmp_path: Path) -> None:
        ...
        error_resp = LLMResponse(
            content="Error calling LLM: provider.BadRequestError: "
...
def test_create_memory_store_uses_redis_uri(tmp_path: Path, monkeypatch) -> None:
    class FakeRedisClient:
        ...
        def ltrim(self, key, start, end):
            ...
            self.data[key] = values[start:end + 1] if values else values

    class FakeRedisModule:
        class Redis:
            @staticmethod
            def from_url(uri, decode_responses=True):
                _ = (uri, decode_responses)
                return FakeRedisClient()

    monkeypatch.setitem(sys.modules, "redis", FakeRedisModule())
    store = create_memory_store(tmp_path, "redis://localhost:6379/0")

    assert isinstance(store, RedisMemoryStore)


def test_create_memory_store_requires_external_backend_when_disabled_fallback(tmp_path: Path) -> None:
    with pytest.raises(ValueError):
        create_memory_store(tmp_path, uri=None, allow_local_fallback=False)


def test_create_memory_store_rejects_unknown_uri_when_disabled_fallback(tmp_path: Path) -> None:
    with pytest.raises(ValueError):
        create_memory_store(tmp_path, uri="mongodb://db:27017/app", allow_local_fallback=False)


def test_create_memory_store_uses_postgresql_uri(tmp_path: Path, monkeypatch) -> None:
    class FakeCursor:
        def __init__(self):
            self._row = None
        ...
        def execute(self, query, params=None):
            ...
            if sql.startswith("select memory_markdown"):
                self._row = ("# Memory",)
            else:
                self._row = None
    ...
    store = create_memory_store(tmp_path, "postgresql://user:pass@localhost:5432/ithqbot")
    assert isinstance(store, PostgreSQLMemoryStore)


def test_create_memory_store_supports_postgresql_plus_driver_scheme(tmp_path: Path, monkeypatch) -> None:
    class FakeCursor:
        def execute(self, query, params=None):
            _ = (query, params)
            return None
    ...
    monkeypatch.setitem(sys.modules, "psycopg", FakePsycopgModule())
    store = create_memory_store(tmp_path, "postgresql+psycopg://user:pass@localhost:5432/ithqbot")
    assert isinstance(store, PostgreSQLMemoryStore)


def test_create_memory_store_fails_fast_after_init_retries(tmp_path: Path, monkeypatch) -> None:
    attempts = {"count": 0}

    def _boom(_workspace, _uri):
        attempts["count"] += 1
        raise RuntimeError("redis unavailable")

    monkeypatch.setitem(_MEMORY_STORE_FACTORIES, "redis", _boom)

    with pytest.raises(RuntimeError, match="External memory store initialization failed"):
        create_memory_store(tmp_path, "redis://localhost:6379/0", allow_local_fallback=True)
    assert attempts["count"] == 3


def test_postgresql_memory_store_wraps_connection_error_with_masked_uri(
    tmp_path: Path, monkeypatch
) -> None:
    def _boom(_self, _uri):
        raise Exception("auth failed")

    monkeypatch.setattr(PostgreSQLMemoryStore, "_connect", _boom)
    with pytest.raises(
        RuntimeError,
        match="memoryStoreUri=postgresql://ithqbot:\\*\\*\\*@127.0.0.1:5432/ithqbot",
    ):
        PostgreSQLMemoryStore(tmp_path, "postgresql://ithqbot:secret@127.0.0.1:5432/ithqbot")
