...
from ithqbot.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        return _response(json={
            "web": {"results": [{"title": "ithqbot", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="ithqbot", count=1)
    assert "ithqbot" in result
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("ithqbot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import ithqbot.agent.tools.web as web_mod
...
@pytest.mark.asyncio
async def test_weather_query_direct_fallback(monkeypatch):
    async def mock_get(self, url, **kw):
        assert "wttr.in" in str(url)
        return httpx.Response(
            200,
            text="Shenzhen: ⛅ +28°C 70% ↙10km/h",
            request=httpx.Request("GET", str(url)),
        )

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="")
    result = await tool.execute(query="深圳天气如何？")
    assert "深圳 的天气" in result
