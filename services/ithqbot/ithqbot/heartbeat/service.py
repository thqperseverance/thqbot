...
class HeartbeatLeaseStore(Protocol):
...
class HeartbeatService:
    ...
    def __init__(
        ...
        enabled: bool = True,
        lease_store: HeartbeatLeaseStore | None = None,
    ...
    async def start(self) -> None:
        ...
        if self.lease_store and self.lease_name:
            logger.info(
                "Heartbeat started (every {}s, lease='{}', ttl={}s)",
                self.interval_s,
                self.lease_name,
                self.lease_ttl_s,
            )
    ...
    async def _tick(self) -> None:
        ...
        if not self._acquire_lease():
            logger.debug("Heartbeat: skipped because another replica holds lease '{}'", self.lease_name)
            return
    ...
    def _acquire_lease(self) -> bool:
        if not self.lease_store or not self.lease_name:
            return True
        now_ms = int(time.time() * 1000)
        ttl_ms = self.lease_ttl_s * 1000
